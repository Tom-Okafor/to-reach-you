  ...
  function moveBall() {
    ...
    ELEMENTS_TO_BLEND_WITH.forEach((element) => {
      DIMENSIONS.push({
        ...
        rightEdge: element.getBoundingClientRect().x + element.offsetWidth,
        topEdge: element.getBoundingClientRect().y + scrollY,
        bottomEdge:
          element.getBoundingClientRect().y + scrollY + element.offsetHeight,
      });
    });
    console.log(DIMENSIONS);
    function isMouseOnBlendElement(mousePositionX, mousePositionY, collection) {
      return (
    ...
    function handleMovement(event) {
      ...
      BALL.style.transform = `translate(${LEFT_POSITION - BALL_WIDTH / 2}px, ${
        TOP_POSITION - BALL_HEIGHT / 2
      }px)`;
    }
  }
